...
TO DO : comment the results : comment is quite similar to without random effects but explained why the model with random effect is better
...
### With random effects
As seen in the previous part, the model we fitted is not completely satisfying, with an AIC that is quite important, a lot of variables included in the regression with some counter-intuitive causal effects, and without taking in account the Tunnel and the Company. Therefore, we want to improve our Poisson model by adding random effects. As for the dataset Fires, we consider :\
...
library('lme4')
model2 <- glmer(Acc~ log(Traffic) + Slope + log(Length) + Limit + Type + (1 | Tunnel) + (1 | Company), family=poisson, data = Accidents)
