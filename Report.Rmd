...
summary(model)
``` 
The model equation we obtained is put below and is quite complicated. There are lots of explanatory variables, and the summary indicates that the AIC is of 6057.6 which is quite enormous. We can surely improve the model on this aspect by reducing the number of regressors. 
$$
...
library('lme4')
model2 <- glmer(Acc~ log(Traffic) + Slope +HGV + log(Length) + Limit + Type + (1 | Tunnel) + (1 | Company), family=poisson, data = Accidents)
...
To avoid having a convergence problem, we rescale the variables and then perform the model. 
...
The residuals look homoscedastic which means that the model explains quite well the data.
...
Here again the residuals are homoscedatic which gives us confidence in the model. All that could be explained has been explained. 


# Conclusion
If we consider models without random effects, the best Poisson model we can build in terms of AIC is the one with the following variables : log(Traffic), SlopeType, log(Length), Limit, Type, Width, Direction, Slope, HGV. This model however has an AIC of 6065 which is quite enormous. \
...
Finally, by rescaling the variables, we obtain a smaller AIC because we obtain a smaller likelihood and the model converges. \
